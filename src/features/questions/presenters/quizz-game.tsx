...
import { QuizzPadHeader } from "./quizz-game-header";
import { QuestionContent } from "./quizz-game-content";
import { QuizzGameOver } from "./quizz-game-over";
...
export function QuizzGame({ questions }: Props) {
    ...
    if (isGameFinished) {
        return <QuizzGameOver onRestart={handleRestart} />;
    }

    return (
        <div className="min-h-screen flex flex-col gap-4 justify-start items-center p-10 max-sm:px-4">
            <QuizzPadHeader progress={progress} maxSeconds={MAX_SECONDS} />
            <QuestionContent
